...
`midas_room` is a separate binary in the same `bin/` directory. Room A (bronze walls, gold key, locked door, red pit) plus Room B through that door, WASD on the floor plane, AABB vs walls, win on B's exit, win/fail overlays, and **R** to restart at A spawn. The camera snaps per room (`Camera::position`). Drawing uses Jim's `room_atlas.bmp` (source-rect cells) with solid-rect fallback. See [GAME_V0.md](GAME_V0.md).

| Input | Action |
| --- | --- |
| **WASD** / arrows | Move |
| Overlap the key | Unlock A's door |
| Overlap the open door | Enter Room B |
| Overlap the exit in B | Win |
| Overlap the red pit | Fail |
| **R** | Restart to Room A (also after a win or fail) |
| **Esc** or close the window | Quit |
