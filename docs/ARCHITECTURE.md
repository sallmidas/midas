...
| `midas_room` | `apps/room/` | v0 game: Room A + Room B, WASD, key, door, hazard |
...
The v0 room app (`midas_room`) is the first game on this API: walls are solids, A's locked door is a solid, key pickup is overlap, the open door is a passage into Room B, overlapping the exit in B wins, and overlapping a stationary hazard fail AABB in A restarts via **R**. The camera snaps per room by writing `Camera::position`. See [GAME_V0.md](GAME_V0.md).
