...
Midas is a C++20 game engine. The public API covers a window, input, a fixed 60 Hz simulation loop with a decoupled present, a 2D renderer with textures (including sprite-sheet source rects) and an orthographic camera, AABB helpers (`aabb_overlap` / `aabb_move`), and lightweight entity/transform helpers. The sandbox is the tech gym: a gold-and-bronze scene you can pan and zoom. `midas_room` is the v0 game: one room, WASD, walls, a key, and a door.
...
apps/room/              v0 game: one room, WASD, key, door, restart
...
| **R** | Restart the room (also after a win) |
| **Esc** or close the window | Quit |

Fixed room camera (identity logical view). Drawing is solid wall rects plus a 3-cell `TextureId` atlas for player / key / door.
...
Room `--smoke` checks `aabb_overlap` / `aabb_move`, walks a scripted key→door path, then runs the same few dummy-video ticks (no BMP required). `ctest` runs both.
...
Not in scope: 3D, ECS, an editor, or multiplayer. Enemies, loot, a generator, and pathfinding are out of game v0.
